...
4. Install runner dependencies once (local setup, not at agent runtime): `npm install` and `npx playwright install chromium` in the `browser-runner/` directory. Then run `npm run provision` with variables from step 3.
